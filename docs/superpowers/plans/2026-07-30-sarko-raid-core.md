...
- **Two `SarkoGame.Build.cs` entries the module cannot compile without**, discovered in Task 1 and root-caused in UBT's own `UEBuildModuleCPP.cs` rather than guessed: `PrivateIncludePaths.Add(ModuleDirectory);` — under V7 `bLegacyPublicIncludePaths` is false, so the module root is not on the include search path and `#include "Core/SarkoRaidSettings.h"` does not resolve. Private is sufficient because every consumer lives in this module. And `"DeveloperSettings"` in `PublicDependencyModuleNames` — `UDeveloperSettings` appears in a header, so it belongs in the public list.
- The automation-test flag spelling that compiles in UE 5.8 is `EAutomationTestFlags::EditorContext | EAutomationTestFlags::EngineFilter`.
- **`GameDefaultMap` must point at `/Engine/Maps/Entry`.** This plan forbids authoring a `.umap`, but the engine cannot start without a level to load — a gap the plan originally missed entirely, which left the game unable to launch at all. The engine's own empty level is the container; every piece of geometry is spawned into it from C++. Set `GameDefaultMap`, `ServerDefaultMap` and `EditorStartupMap` in `Config/DefaultGame.ini`.
